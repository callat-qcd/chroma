...
namespace Chroma
{
  ...
  public:
     ...
     * \param j_decay       direction of decay ( Read )
     * \param obsvP         compute currents and residual mass ( Read )
     * \param q_mp_id       if nonempty, named object id to store midpoint prop
     * \param ncg_had       number of solver iterations ( Write )
     */
    virtual void quarkProp(typename PropTypeTraits<T>::Type_t& q_sol,
			   XMLWriter& xml_out,
			   ...
			   int& ncg_had) const
      {
	quarkProp(q_sol, xml_out, q_src, t_src, j_decay, state, invParam, quarkSpinType, obsvP, ncg_had);
      }
